...
public class HashMap<K,V> extends AbstractMap<K,V>
    ...
    final Node<K,V>[] resize() {
        Node<K,V>[] oldTab = table;  //旧数据做个副本
        int oldCap = (oldTab == null) ? 0 : oldTab.length;//槽位
        int oldThr = threshold; //边界值
        int newCap, newThr = 0;
        if (oldCap > 0) {
            if (oldCap >= MAXIMUM_CAPACITY) {  //槽位>= 最大槽位值 无法再扩容了
                threshold = Integer.MAX_VALUE;
                return oldTab;
            }else if ((newCap = oldCap << 1) < MAXIMUM_CAPACITY && oldCap >= DEFAULT_INITIAL_CAPACITY) { //已经初始化,新槽位扩容(*2)后 小于最大槽位
                newThr = oldThr << 1; // 边界值 扩大(*2)
            }
        }else if (oldThr > 0) // 无槽位，但是有边界值，将槽位 = 边界值
            newCap = oldThr;
        else {               // 使用默认槽位和边界值
            newCap = DEFAULT_INITIAL_CAPACITY;  //16  1>>4
            newThr = (int)(DEFAULT_LOAD_FACTOR * DEFAULT_INITIAL_CAPACITY);  // 0.75*16  边界值
        }
        if (newThr == 0) {   //无边界值   ？不知道什么时候会走到这
            float ft = (float)newCap * loadFactor;  //loadFactor？
            newThr = (newCap < MAXIMUM_CAPACITY && ft < (float)MAXIMUM_CAPACITY ?(int)ft : Integer.MAX_VALUE);
        }
        threshold = newThr;
        Node<K,V>[] newTab = (Node<K,V>[])new Node[newCap];
        table = newTab;  //将table -> 新的
        if (oldTab != null) {  //旧数据迁移到新table中
            for (int j = 0; j < oldCap; ++j) {
                Node<K,V> e;
                if ((e = oldTab[j]) != null) {
                    oldTab[j] = null; //设置为空，方便回收
                    if (e.next == null) {  //该槽位只有一个值（无链式）
                        newTab[e.hash & (newCap - 1)] = e;  //将该值散列到新数组中
                    }else if (e instanceof TreeNode) {  //该值未树形结构
